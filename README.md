...
## Decompose DDB1 motion and contact effects

The directional-mechanics workflow compares isolated CRBN with fixed,
rigid-body and flexible DDB1 boundary conditions in the same CRBN internal
coordinate space. It separates directional compliance from mean compliance,
tracks assembly modes as interface strength changes, and evaluates the frozen
residue–contact groups under each partner condition.

Use the matching directional-mechanics data bundle, supplied separately from
this code-only repository:

```bash
python scripts/stage_directional_bundle.py /path/to/CRBN_directional_mechanics_data_bundle_20260906.zip
python scripts/run_directional_mechanics.py --config scripts/directional_config.json --output-dir results/directional_mechanics --offline
```

The stager verifies every member against the bundle manifest and restores
root-level inputs plus cached external data and analysis tables. It verifies
the included code snapshot without replacing this checkout's code. The frozen
configuration covers five open references, three cutoffs and two spring
weightings (30 conditions). Mode tracking stores 60 nonzero modes at each of
21 interface strengths; the primary rank comparison searches 20 modes.
The large generated mode-vector arrays are inventoried in the bundle and
regenerated by the workflow.

To run selected stages:

```bash
python scripts/run_directional_mechanics.py --offline --stages mechanics contacts
python scripts/run_directional_mechanics.py --offline --stages modes external figures
```

Outputs under `analysis/mechanics/`, `analysis/contact_roles/` and
`analysis/mode_paths/` include per-condition inputs, completion hashes and
CSV/JSON source tables. A completed stage is reused only when its recorded
inputs, settings, code and output hashes match. The figure stage writes
PNG/PDF/SVG files under `manuscript/figures/` and exact plotting tables under
`analysis/directional_figure_sources/`. It also uses the inherited SAXS and
within-assay comparison tables supplied in the bundle.

All four static models use the same 801-dimensional CRBN internal space.
Effective stiffness eigenvalues are static quantities. Joint-network mode
ranks describe the larger assembly; primary matches use the CRBN component
after removing receptor translation and rotation. Explicit `raw_*` fields
retain the unprojected comparison. Relative closure compliance is directional
compliance divided by mean internal compliance. Its geometric percentiles
refer to the specified family of local rotations. Contact-strength effects
are model predictions and do not specify the effect of an amino-acid mutation.

The external stage preserves source URLs, acquisition status and hashes. It
compares all frozen candidates with the deposited allosteric site and acquired
variant observations. Spatial overlap and qualitative functional outcomes
