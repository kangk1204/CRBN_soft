...
PLAN_D_FIT_TEMPLATES = {
    "open_8cvp_crbn": ROOT / "data" / "_cif_cache" / "8CVP.cif.gz",
    "closed_5fqd_crbn": ROOT / "data" / "_cif_cache" / "5FQD.cif.gz",
}
...
def find_chimerax() -> dict[str, Any]:
    candidates = [
        ...
        *sorted(str(path) for path in Path("/Applications").glob("ChimeraX-*.app/Contents/MacOS/ChimeraX")),
    ]
    ...
    *,
    ...
    return {
        ...
        "claim_boundary": (
            ...
            "remain fitted models, never experimentally deposited atomic models. Quantitative "
            "directional comparison requires half-map, local-density and template-stability checks."
        ),
    }
...
MIN_CHIMERAX_VERSION = (1, 12)


def _candidate_chimerax() -> list[Path]:
    candidates: list[str | None] = [
        os.environ.get("CRBN_CHIMERAX"),
        shutil.which("ChimeraX"),
        shutil.which("chimerax"),
        "/Applications/ChimeraX.app/Contents/MacOS/ChimeraX",
        "/Applications/UCSF ChimeraX.app/Contents/MacOS/ChimeraX",
    ]
    candidates.extend(str(path) for path in Path("/Applications").glob("ChimeraX-*.app/Contents/MacOS/ChimeraX"))
    seen: set[Path] = set()
    existing: list[Path] = []
...
def _chimerax_version(executable: Path) -> tuple[int, int] | None:
    try:
        ...
        return None
    match = re.search(r"(?:ChimeraX\\s*)?(\\d+)\\.(\\d+)", completed.stdout)
    if not match:
        return None
...
def _runtime_cxc(asset_dir: Path) -> Path:
    runner = asset_dir / "chimerax_plan_d_runner.py"
    config = asset_dir / "chimerax_plan_d_config.json"
    log = asset_dir / "chimerax_plan_d_log.html"
    ...
    with handle:
        handle.write(
            "# Runtime ChimeraX script generated by run_chimerax_plan_d.py\\n"
            f"open {_quote(runner)}\\n"
            f"crbnpland {_quote(config)}\\n"
            f"log save {_quote(log)}\\n"
            "exit\\n"
        )
...
def main() -> int:
    ...
    candidates = _candidate_chimerax()
    if not candidates:
        sys.stderr.write("No ChimeraX executable found. Set CRBN_CHIMERAX to the ChimeraX binary.\\n")
        return 2
    executable = candidates[0]
    version = _chimerax_version(executable)
    if version is None or version < MIN_CHIMERAX_VERSION:
        sys.stderr.write(
            f"ChimeraX 1.12 or newer is required; found {version or 'unknown'} at {executable}.\\n"
        )
        return 2
    runtime_cxc = _runtime_cxc(asset_dir)
    try:
        return subprocess.run(
            [str(executable), "--nogui", "--cmd", f"open {_quote(runtime_cxc)}"],
            check=False,
        ).returncode
    finally:
...
def plan_d_reference_sources() -> dict[str, Path]:
    return {
        ...
        "open_8cvp.cif.gz": PLAN_D_FIT_TEMPLATES["open_8cvp_crbn"],
        "closed_5fqd.cif.gz": PLAN_D_FIT_TEMPLATES["closed_5fqd_crbn"],
    }
...
def plan_d_config(
    ...
    *,
        ...
        targets.append(
            {
                ...
                "focus_map": (path_from_config_dir(output_root / "data/maps" / f"EMD-{focus}" / f"emd_{focus}.map.gz", analysis_root)
                              if (focus := {"70776": "70777", "70782": "70783"}.get(emdb_id)) else None),
            }
        )
    reference_inputs = {
        ...
        "open_8cvp.cif.gz": "reference_inputs/open_8cvp.cif.gz",
        "closed_5fqd.cif.gz": "reference_inputs/closed_5fqd.cif.gz",
    }
    ...
    return {
        ...
        "fit_templates": {
            "open_8cvp_crbn": reference_inputs["open_8cvp.cif.gz"],
            "closed_5fqd_crbn": reference_inputs["closed_5fqd.cif.gz"],
        },
        "template_chain": "B",
        ...
        "output_dir": "chimerax_plan_d",
        "fit_output_subdir": "chimerax_plan_d",
        "method_boundary": (
...
def write_chimerax_assets(output_root: Path, analysis_root: Path) -> None:
    ...
    launcher_py = analysis_root / "run_chimerax_plan_d.py"
    ...
    atomic_write_text(launcher_py, CHIMERAX_PLAN_D_LAUNCHER)
    launcher_py.chmod(0o755)
    atomic_write_text(
        run_cxc,
        """# ChimeraX Plan D run script for CRBN domain rigid fitting.
# Requires UCSF ChimeraX installed after user/licensee acceptance of the UCSF license.
# Run example:
#   cd path/to/analysis/maps
#   ChimeraX --nogui --cmd "open run_chimerax_plan_d.cxc"
# Portable launcher:
#   python3 run_chimerax_plan_d.py

open "chimerax_plan_d_runner.py"
crbnpland "chimerax_plan_d_config.json"
log save "chimerax_plan_d_log.html"
exit
""",
    )
    atomic_write_text(
        install_md,
        """# ChimeraX execution helper

Run `python3 run_chimerax_plan_d.py` from this directory. The launcher resolves
`chimerax_plan_d_runner.py` and `chimerax_plan_d_config.json` relative to this
directory, writes a temporary local ChimeraX command script, checks for
ChimeraX 1.12 or newer, and then runs ChimeraX in `--nogui` mode.

The plain `run_chimerax_plan_d.cxc` file is also kept relocatable: it contains
only paths relative to this directory and can be opened manually after changing
to this directory.

If ChimeraX is installed outside the usual macOS or PATH locations, set
`CRBN_CHIMERAX` to the ChimeraX executable. The runner requires staged reference
inputs under `analysis/maps/reference_inputs`; `chimerax_plan_d_config.json`
records `missing_reference_inputs` if a public checkout does not contain those
frozen inputs. Run the fitting only when `reference_inputs_available is true`
and `missing_reference_inputs` is empty.
""",
    )
